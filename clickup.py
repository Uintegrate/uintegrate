...
def clickup_create_task_dependency(creds,params):
    ...
    :param dict params: contains properties to be added to the created task dependency
        ...
        - :depends_on: (str, Required) Use the depends_on parameter in the request body to specify the task that must be completed before the task in the path parameter.
        - :depedency_of: (str, Required) Use the dependency_of parameter in the request body to specify the task that's waiting for the task in the path parameter to be completed.
        - :custom_task_ids: (bool) If you want to reference a task by it's custom task id, this value must be true.
    ...
    try:
        token = extract_credentials(creds)
        required_params=["task_id","depends_on", "dependency_of"]
        if all(param in params for param in required_params):
            dependency = {}
