...
def create_api_instance(cred):
    ...
    try:
        if 'accesstoken' in cred:
            api = Api(cred['accesstoken'], retry_strategy=retry_strategy(total=10))
            return api
        else:
...
def create_table_instance(cred, base_id, table_id):
    ...
    """
    try:
        if 'accesstoken' in cred and base_id and table_id:
            api = create_api_instance(cred)
...
def airtable_list_records(cred, params):
    ...
    :ACCESS_TOKEN: String used for authentication purposes.
    ...
    try:
        ...
        table = create_table_instance(creds, base_id, table_id)
...
def airtable_create_record(cred, params):
    ...
    try:
        ...
        if "fields" in params:
            ...
            table = create_table_instance(creds['accesstoken'], base_id, table_id)
